...
describe("StakingRewards", function () {
  ...
  before(async function () {
    ...
    stakingToken = await AgentToken.deploy("StakingToken", "STK", ethers.parseEther("100000000"));
    await stakingToken.waitForDeployment();

    rewardToken = await AgentToken.deploy("RewardToken", "RWD", ethers.parseEther("100000000"));
    await rewardToken.waitForDeployment();

    const StakingRewards = await ethers.getContractFactory("StakingRewards");
    stakingRewards = await StakingRewards.deploy(await stakingToken.getAddress(), await rewardToken.getAddress());
    ...
    // Notify reward amount to set rewardRate and periodFinish
  ...
  it("should distribute rewards correctly to multiple stakers", async function () {
    ...
    await stakingToken.connect(staker2).approve(await stakingRewards.getAddress(), amount);
