...
describe("PrizeSplit Dust Test", function () {
  ...
  beforeEach(async function () {
    ...
    await owner.sendTransaction({
      to: prizeSplit.address,
      value: ethers.utils.parseEther("1.001"),
    });
  });

  it("allocates dust to last winner", async function () {
    const initialBalanceWinner1 = await ethers.provider.getBalance(
      winner1.address
    );
    const initialBalanceWinner2 = await ethers.provider.getBalance(
      winner2.address
    );

    await prizeSplit.claimPrize(
      [winner1.address, winner2.address],
      [ethers.utils.parseEther("0.5"), ethers.utils.parseEther("0.5")]
    );

    const finalBalanceWinner1 = await ethers.provider.getBalance(
      winner1.address
    );
    const finalBalanceWinner2 = await ethers.provider.getBalance(
      winner2.address
    );

    expect(finalBalanceWinner2.sub(initialBalanceWinner2)).to.equal(
