...
// ---------------------------------------------------------------------------
// Compile encoding.ts to a temp directory so we can import the JS output
// ---------------------------------------------------------------------------
...
assert.ok(
  compiledEncoding && existsSync(compiledEncoding),
  "compiled encoding.js was emitted",
);
...
// ---------------------------------------------------------------------------
// 1. Backwards compatibility — static-slot decoders still work
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 2. String + bytes dynamic values
// ---------------------------------------------------------------------------

test("decodeParameter decodes string and bytes dynamic ABI values", () => {
  // String decoding
  assert.equal(
    encoding.decodeParameter(
      "string",
      coder.encode(["string"], ["hello agent"]),
    ),
    "hello agent",
  );

  // Bytes decoding → Buffer
  const decodedBytes = encoding.decodeParameter(
    "bytes",
    coder.encode(["bytes"], ["0x1234abcd"]),
  );
...
test("decodeParameter decodes dynamic arrays with static and dynamic elements", () => {
  ...
  assert.deepEqual(
    encoding.decodeParameter(
      "string[]",
      coder.encode(["string[]"], [["alpha", "beta"]]),
    ),
    ["alpha", "beta"],
  );
});
...
// ---------------------------------------------------------------------------
// 4. Complex return type: string + array + uint (acceptance criteria test)
// ---------------------------------------------------------------------------
...
test("decodeParams decodes required complex return type with string array and uint", () => {
  ...
  assert.deepEqual(
    encoding.decodeParams(["string", "uint256[]", "uint256"], encoded),
    ["task-ready", [5n, 8n, 13n], 21n],
  );
});

// ---------------------------------------------------------------------------
// 5. Byte offsets for mixed return data
// ---------------------------------------------------------------------------
...
test("decodeParameter supports byte offsets for dynamic values in mixed return data", () => {
  ...
  assert.equal(
    encoding.decodeParameter("string", encoded, 32),
    "offset string",
  );
});

// ---------------------------------------------------------------------------
// 6. Nested tuple decoding (recursive)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// 7. Named tuple descriptors (struct-like returns)
// ---------------------------------------------------------------------------
