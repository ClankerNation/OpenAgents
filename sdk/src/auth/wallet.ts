...
export class Wallet {
  ...
  async signTransaction(tx: Transaction): Promise<SignedTransaction> {
    ...
    if (tx.maxFeePerGas !== undefined && tx.maxPriorityFeePerGas !== undefined) {
      ...
      const txHash = keccak256(encodeParams(txFields.map((f, i) => ({
        type: "uint256",
        value: f,
      }))));
      ...
      // Legacy transaction
      txType = 0;
      chainIdForSig = BigInt(tx.chainId ?? 1);
  ...
  private parseSignature(sig: string): { r: bigint; s: bigint; v: number } {
    ...
    const key = secp256k1.recoverPubKeyFromRecoveryParam(hash.slice(0, 32), hash[32], hash[33]);
