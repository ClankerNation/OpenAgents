...
contract YieldAggregator is Ownable, ReentrancyGuard {
    ...
    function addStrategy(address target, uint256 maxBps) external onlyOwner {
        require(target != address(0), "Vault: zero target");
        require(maxBps > 0 && maxBps <= MAX_BPS, "Vault: invalid maxBps");
    ...
    function setStrategyMaxAllocation(uint256 strategyId, uint256 maxBps) external onlyOwner {
        require(maxBps > 0 && maxBps <= MAX_BPS, "Vault: invalid maxBps");
        strategies[strategyId].maxAllocationBps = maxBps;
    }
    ...
    function allocate(uint256 strategyId, uint256 amount) external onlyOwner {
        ...
        uint256 cap = (totalAssets() * s.maxAllocationBps) / MAX_BPS;
        require(s.allocated + amount <= cap, "Vault: exceeds max allocation");
    ...
    function rebalance(uint256 fromId, uint256 toId, uint256 amount) external onlyOwner {
        Strategy storage from = strategies[fromId];
        Strategy storage to = strategies[toId];
        require(from.active && to.active, "Vault: inactive strategy");
        require(from.allocated >= amount, "Vault: insufficient allocated");

        from.allocated -= amount;
        uint256 cap = (totalAssets() * to.maxAllocationBps) / MAX_BPS;
        require(to.allocated + amount <= cap, "Vault: exceeds max allocation");
        to.allocated += amount;
    }
