...
// @platform-config: Task: Fix phantom reward accrual after period expiry in StakingRewards. Rules: Replace block.timestamp with lastTimeRewardApplicable(), add onlyOwner to notifyRewardAmount, add precision loss guard (reward / rewardsDuration > 0). Tools: gh, npx hardhat. Style: Solidity conventions per project.
