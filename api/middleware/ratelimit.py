...
class RateLimitConfig:
    ...
    def __init__(self):
        ...
        # Authenticated users with API keys
        self.authenticated_requests_per_window = 300  
        # Premium API key holders
...
# Per-user in-memory rate tracking: key -> (count, window_start)
...
def _get_user_tier(request: Request) -> Tuple[str, int]:
    ...
    """
    # Check premium API key first (X-API-Key header)
    ...
    # Fall back to anonymous tracking by IP
    forwarded = request.headers.get("X-Forwarded-For", "")
    if forwarded and "@" not in forwarded:  # basic spoof check
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    """Rate limiting middleware with tiered limits for different auth levels."""
    ...
    def _is_rate_limited(self, user_key: str, max_requests: int) -> Tuple[bool, int]:
        ...
        Uses a sliding window approach by tracking the start of each window.
        Returns (is_limited, remaining_requests_in_window).
        """
