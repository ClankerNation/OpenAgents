"""
 ...
 @runtime os: Linux 6.17.0-PRoot-Distro, arch: aarch64, working_dir: /root/OpenAgents, shell: /usr/bin/bash
 """
...
async def deposit_escrow(
    ...
    base_time = deposit.releaseTime or task.deadline or datetime.utcnow()
    ...
    db.commit()
    db.refresh(payment)
    return {"payment_id": payment.id, "status": "escrowed", "amount": payment.amount, "expired_at": payment.expired_at.isoformat() if payment.expired_at else None}
...
async def process_expired_escrows(db=Depends(get_db)):
    """Finds and auto-refunds all escrowed funds that are 30 days past releaseTime."""
    now = datetime.utcnow()
    # Filter memory-side for computed property expired_at
    # Since release_time + 30 days = expired_at, in SQL we can just do:
    cutoff_time = now - timedelta(days=30)
    
    expired_payments = db.query(Payment).filter(
        Payment.status == "escrowed",
        Payment.release_time != None,
        Payment.release_time <= cutoff_time
    ).all()
    
    refunded_count = 0
    for payment in expired_payments:
        # Extra safety check using the computed property
        if payment.expired_at and payment.expired_at <= now:
            payment.status = "refunded"
            payment.to_address = payment.from_address
            payment.claimed_at = now
            refunded_count += 1
            logger.info(f"Auto-refunded expired escrow: ID {payment.id}, Task {payment.task_id}, Timestamp {now.isoformat()}")
            
    if refunded_count > 0:
        db.commit()
        
    return {"status": "success", "processed_count": refunded_count}
