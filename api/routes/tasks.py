...
async def get_task(task_uuid: str, db=Depends(get_db)):
    ...
    return {"id": task.uuid, "title": task.title, "description": task.description, "status": task.status, "reward_amount": task.reward_amount, "created_at": task.created_at.isoformat() if task.created_at else None}
