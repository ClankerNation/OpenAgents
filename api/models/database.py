...
class User(Base):
    ...
    address = Column(String(42), unique=True, nullable=False, index=True)  # Added index for auth lookups
    username = Column(String(64), unique=True, nullable=True)
    # BUG: No index on address — wallet lookups on every auth request do full table scans
    created_at = Column(DateTime, default=lambda: datetime.now(timezone.utc))  # BUG: naive datetime, no timezone
